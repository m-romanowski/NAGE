...
namespace mr::nage
{
    ...
    void Game::initializeScene()
    {
        ...
        skybox = std::make_shared<DayNightSkydome>("Atmospheric scattering skydome", skyboxShader.get(), sun.get(), 6371000);
        sceneManager()->sceneByKey("world")->addToScene(skybox.get());
    ...
    void Game::ioEventsSupplier()
    {
        ...
        float xForSun = std::cos(counterForSun * DEG2RAD) * skybox->radius();
        float yForSun = std::sin(counterForSun * DEG2RAD) * skybox->radius();
