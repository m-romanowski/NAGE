...
vec3 combineColorFromTexturesRGB()
{
    vec4 blendMapText = texture(material.blendmap, fragmentTextureCoords);

    float backTextureAmount = 1.0f - (blendMapText.r + blendMapText.g + blendMapText.b);
    vec2 tiledCoords = fragmentTextureCoords * 30.0f;

    vec4 dirtTexture = texture(material.dirt, tiledCoords) * backTextureAmount;
    vec4 grassTexture = texture(material.grass, tiledCoords) * blendMapText.r;
    vec4 stoneTexture = texture(material.stone, tiledCoords) * blendMapText.g;
    vec4 snowTexture = texture(material.snow, tiledCoords) * blendMapText.b;
...
vec4 combineColorFromTexturesRGBA()
{
    vec4 blendMap = texture(heightMapTexture, fragmentTextureCoords);

    float backTextureAmount = 1.0f - (blendMap.r + blendMap.g + blendMap.b);
    vec2 tiledCoords = fragmentTextureCoords * 40.0f;

    vec4 dirtTexture = texture(material.dirt, tiledCoords) * backTextureAmount;
    vec4 grassTexture = texture(material.grass, tiledCoords) * blendMap.r;
    vec4 stoneTexture = texture(material.stone, tiledCoords) * blendMap.g;
    vec4 snowTexture = texture(material.snow, tiledCoords) * blendMap.b;
